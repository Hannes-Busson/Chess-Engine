...
mod tests {
    ...
    use crate::movegen::{Move, MoveFlags, MoveGen};
    ...
    fn has(moves: &[Move], from: u8, to: u8, flag: u8) -> bool {
        moves.iter().any(|m| m.from() == from && m.to() == to && m.flags() == flag)
    }
    ...
    fn starting_position_move_count() {
        let moves = MoveGen::generate_moves(Position::start());
    ...
    fn black_double_push_sets_ep_square() {
        ...
        place(&mut pos, Color::Black, PieceType::Pawn, sq(4, 6)); // e7
        let moves = MoveGen::generate_moves(pos);
        let dpp = *moves.iter().find(|m| m.flags() == MoveFlags::DOUBLE_PAWN_PUSH).unwrap();
        let new_pos = pos.make_move(dpp);
    ...
    fn make_move_queen_promotion() {
        ...
        place(&mut pos, Color::White, PieceType::Pawn, sq(4, 6)); // e7
        let moves = MoveGen::generate_moves(pos);
        let qp = *moves.iter().find(|m| m.flags() == MoveFlags::QUEEN_PROMOTION).unwrap();
        ...
        assert_eq!(new_pos.piece_on[sq(4, 7) as usize], 0 * 6 + 4, "piece_on e8 should be white queen");
    }
    ...
    fn castle_blocked_by_piece_in_path() {
        ...
        let moves = MoveGen::generate_moves(pos);
        assert!(
            !moves.iter().any(|m| m.flags() == MoveFlags::KINGSIDE_CASTLE),
    ...
    fn castling_rights_cleared_after_king_move() {
        ...
        place(&mut pos, Color::Black, PieceType::King, sq(4, 7));
        let moves = MoveGen::generate_moves(pos);
        let km = *moves.iter().find(|m| m.from() == sq(4, 0) && m.flags() == MoveFlags::QUIET).unwrap();
    ...
    fn castling_rights_cleared_after_rook_h1_move() {
        ...
        place(&mut pos, Color::Black, PieceType::King, sq(7, 7));
        let moves = MoveGen::generate_moves(pos);
        let rm = *moves.iter().find(|m| m.from() == sq(7, 0) && m.flags() == MoveFlags::QUIET).unwrap();
        let new_pos = pos.make_move(rm);
        assert_eq!(new_pos.castling & CastlingRights::WK, 0, "WK should be cleared after h1 rook moves");
    }
    ...
    fn make_move_capture_updates_piece_on() {
        ...
        assert_eq!(new_pos.piece_on[sq(0, 6) as usize], 0 * 6 + 3, "a7 should have white rook");
    ...
    fn is_attacked_by_knight() {
        ...
        assert!(MoveGen::is_attacked(sq(4, 0), &pos), "e1 should be attacked by knight on f3");
        assert!(!MoveGen::is_attacked(sq(3, 0), &pos), "d1 should not be attacked");
    }
    ...
    fn is_attacked_by_pawn() {
        ...
        assert!(MoveGen::is_attacked(sq(4, 0), &pos), "e1 should be attacked by pawn on d2");
    }
    ...
    fn is_attacked_by_bishop() {
        ...
        assert!(MoveGen::is_attacked(sq(4, 0), &pos), "e1 should be attacked by bishop on b4");
    }
    ...
    fn is_attacked_by_rook() {
        ...
        assert!(MoveGen::is_attacked(sq(4, 0), &pos), "e1 should be attacked by rook on e6");
    }
    ...
    fn bishop_attacks_d4_empty_board() {
        let attacks = MoveGen::bishop_attacks(sq(3, 3), Bitboard(0)); // d4
        assert_eq!(attacks.0.count_ones(), 13, "bishop d4 empty board: expected 13, got {}", attacks.0.count_ones());
    }

    #[test]
    fn rook_attacks_a1_empty_board() {
        let attacks = MoveGen::rook_attacks(sq(0, 0), Bitboard(0));
        assert_eq!(attacks.0.count_ones(), 14, "rook a1 empty board: expected 14, got {}", attacks.0.count_ones());
    }
    ...
    fn rook_blocker_stops_ray_but_includes_capture_square() {
        ...
        let attacks = MoveGen::rook_attacks(sq(0, 0), occ); // rook on a1
